            ...
            for event_id, data in events_to_handle:
                ...
                if should_cleanup:
                    ...
                    if voice_channels:
                        ...
                        if len(voice_channels) == 2:
                            ...
                            if isinstance(voice1, discord.VoiceChannel):
                                for member in voice_channels[0].members:
                                    try:
                                        await member.move_to(voice1, reason="Role deleted, moving to lobby")
                            ...
                            if isinstance(voice2, discord.VoiceChannel):
                                for member in voice_channels[1].members:
                                    try:
                                        await member.move_to(voice2, reason="Role deleted, moving to lobby")
                                    except (discord.Forbidden, discord.HTTPException):
                                        pass
                        else:
                        ...
                        for vc in voice_channels:
                            try:
                                await vc.delete(reason=f"Event role '{role.name}' was deleted")
