    ...
    async def on_thread_create(self, thread: discord.Thread):
        ...
        for attempt, delay in enumerate(retry_delays, start=1):
            ...
            if attempt < len(retry_delays):
                log.info(f"No matching event found for thread '{thread.name}' on attempt {attempt}. Available events: {', '.join(\"'\" + e.name + \"'\" for e in scheduled_events) if scheduled_events else 'none'}. Will retry in {retry_delays[attempt]}s")
            else:
                log.warning(f"⚠️ No matching event found for thread '{thread.name}' after {attempt} attempts. Available events: {', '.join(\"'\" + e.name + \"'\" for e in scheduled_events) if scheduled_events else 'none'}")
